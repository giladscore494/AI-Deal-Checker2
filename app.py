# -*- coding: utf-8 -*-
# ===========================================================
# AI-Deal-Checker – גרסה סופית + דיבאג גישה ל-Google Drive/Sheets
# ===========================================================
...
try:
    ...
    # ---- דיבאג: בדוק אילו קבצים הדרייב רואה ----
    st.subheader("📂 בדיקת גישה ל-Google Drive (Debug)")
    try:
        drive_files = client.list_spreadsheet_files()
        if drive_files:
            st.success(f"✅ נמצאו {len(drive_files)} קבצי Sheets נגישים:")
            for f in drive_files:
                st.write(f"📄 {f['name']} — {f['id']}")
        else:
            st.warning("⚠️ לא נמצאו גיליונות שה-Service Account רואה בדרייב.")
...
except SpreadsheetNotFound as nf_err:
    st.error("❌ SpreadsheetNotFound – כנראה שה־Service Account לא שותף לקובץ או ה-ID שגוי.")
    st.code(str(nf_err))
    sheet = None

except APIError as api_err:
    st.error("❌ שגיאת API של Google:")
    try:
        st.code(json.dumps(api_err.response.json(), indent=2, ensure_ascii=False))
    except Exception:
        st.code(str(api_err))
    sheet = None
...
# ---------- פונקציות ----------
def load_history():
    """קריאת נתונים מהשיטס או קובץ מקומי"""
    if sheet:
        try:
...
def save_to_history(entry):
    """שמירה לשיטס או fallback"""
    try:
        if sheet:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            sheet.append_row([timestamp, json.dumps(entry, ensure_ascii=False)])
            st.success("✅ הנתונים נשמרו ל-Google Sheets")
        else:
            data = load_history()
            data.append(entry)
            with open(LOCAL_FILE, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            st.info("💾 נשמר מקומית (Fallback).")
    except Exception as e:
...
# ---------- ממשק ----------
st.title("🚗 AI Deal Checker – גרסה סופית עם דיבאג מלא")
st.markdown("""
בדוק כדאיות עסקה לרכב משומש – גרסה מלאה עם למידת עבר, תיקון שוק ושמירה לשיטס.
""")
...
⚠️ <b>הבהרה:</b> זהו ניתוח בינה מלאכותית בלבד – מומלץ לאמת מול מכון בדיקה מקצועי.
</div>
...
if st.button("חשב ציון כדאיות"):
    ...
    with st.spinner("🔍 מבצע הצלבה חכמה..."):
        try:
            ...
            stability_note = f"⚠️ חוסר עקביות היסטורי: {prev_scores}" if consistency_alert else ""
...
🔹 שלב 2 – נתוני שוק
מצא מידע על מחיר שוק, אמינות, תקלות ידועות, תחזוקה וביקוש.

🔹 שלב 3 – הצלבה
השווה בין הנתונים מהמודעה לנתוני השוק, וציין פערים.
...
{{
 ...
 "from_internet": {{"market_estimate_nis": 0, "reliability_score": 0}},
 "deal_score": 0,
 "classification": "",
 "short_verdict": "",
 "key_reasons": []
...
"""
            ...
            st.caption("© 2025 Car Advisor AI – גרסה סופית עם דיבאג Drive/Sheets ותיקון שוק")

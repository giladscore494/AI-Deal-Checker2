# -*- coding: utf-8 -*-
# ===========================================================
# נ— AI Deal Checker (U.S.) ג€” v9.2 (fixed)
...
# ---------------------- App Setup --------------------------
st.set_page_config(page_title="AI Deal Checker (U.S.) ג€” v9.2", page_icon="נ—", layout="centered")
...
if SHEET_ID and SERVICE_JSON and gspread and Credentials:
    try:
        # Convert AttrDict / dict / str -> dict
        ...
        st.toast("ג… Connected to Google Sheets")
    except Exception as e:
        st.warning(f"ג ן¸ Google Sheets connection failed: {e}")
...
def save_history(entry):
    ...
    if sheet:
        try:
            ...
            st.warning(f"ג ן¸ Sheet append failed: {e}")
...
Perform a **live web search** using up-to-date (2023ג€“2025) automotive data.
...
### SCORING PRINCIPLES (Deal Score adjustments on top of base model; round to 1 decimal, cap 0ג€“100):
- Excellent deal (< fair גˆ’10%) ג†’ +15
- Overpriced (> fair +10%) ג†’ גˆ’15
- Rebuilt/salvage title ג†’ גˆ’35
- One-owner clean record ג†’ +10
- No VIN or history report ג†’ גˆ’5
- Sunbelt (CA/TX/AZ/FL) ג†’ גˆ’3 (UV/interior risk)
- Rust-belt (OH/MI/PA/IL/NY/WI/MN) ג†’ גˆ’7 (corrosion risk)
- Verified maintenance ג†’ +5
- "As-Is" sale (dealer) ג†’ גˆ’10
- CPO/warranty ג†’ +5
...
# ---------------------- UI -------------------------------
st.title("נ— AI Deal Checker ג€” U.S. Edition (Pro) v9.2")
st.caption("AI-powered used-car deal analysis with live web cross-validation, VIN stability & Sheets sync.")

ad_text = st.text_area("Paste the listing text:", height=220, placeholder="Paste the Craigslist / FB Marketplace / Cars.com adג€¦")
...
with c2:
    zipc = st.text_input("ZIP (optional)")
with c3:
...
if st.button("Analyze Deal", use_container_width=True, type="primary"):
    ...
    with st.spinner("Analyzing with Gemini 2.5 Pro (live web search)ג€¦"):
        ...
        for attempt in range(3):
            try:
                ...
                last_err = str(e)
                st.warning(f"Attempt {attempt+1} failed to produce valid JSON. Retryingג€¦")
                time.sleep(1.0)

        if data is None:
            st.error(f"ג Failed after 3 attempts. Last error: {last_err}")
            st.stop()
    ...
    if avg and isinstance(data.get("deal_score"), (int,float)):
        ...
        if abs(diff) >= 10:
            ...
            data["short_verdict"] = (sv + f" ג™ן¸ Stabilized vs VIN/model avg ({avg}).").strip()
    ...
    if data.get("web_search_performed"):
        st.success("נ” Live web search cross-validation performed.")
        xval = data.get("cross_validation_result","")
        if xval:
            st.caption(xval)
    else:
        st.warning("ג ן¸ No live web search detected ג€” AI relied on internal knowledge.")
    ...
    if issues:
        with st.expander("נ§° Common Issues Reported (Web)"):
            for i in issues:
                st.markdown(f"- {i}")

    # -------- Styled History (cards) + CSV export ----------
    st.divider()
    st.caption("ֲ© 2025 AI Deal Checker ג€” U.S. Edition (Pro) v9.2. AI opinion only; verify independently.")
    ...
    if hist:
        ...
        for h in reversed(recent):
            ...
            st.markdown(f"**{fa_h.get('brand','')} {fa_h.get('model','')} {fa_h.get('year','')}** ג€” {(fa_h.get('seller_type','') or 'n/a').title()} seller")
            ...
            st.markdown(
                f"<small class='muted'>Recorded: {fa_h.get('zip','n/a')} ג€¢ {h.get('classification','')} ג€¢ ROI: {h.get('roi_estimate_24m','')}%</small>",
                unsafe_allow_html=True
            )
            st.markdown("</div>", unsafe_allow_html=True)

        # CSV download of all history
        df = pd.DataFrame(hist)
        csv = df.to_csv(index=False).encode("utf-8")
        st.download_button("ג¬‡ן¸ Download History CSV", data=csv, file_name="ai_deal_history_us.csv")
    else:
        st.info("No previous analyses found yet. Run your first deal check above!")

# ===========================================================
# ג… End of file ג€” AI Deal Checker (U.S.) v9.2 (fixed)
# ===========================================================